...
const handler = async (event) => {
    ...
    try {
        ...
        const startDate = (0, apiHelpers_1.getQueryParameter)(event, 'startDate');
        const endDate = (0, apiHelpers_1.getQueryParameter)(event, 'endDate');
        if (!startDate || !endDate) {
            return (0, apiHelpers_1.createErrorResponse)(400, 'startDate and endDate are required');
        }
        ...
        const bookings = await layers_1.Booking.find({
            therapistId,
            date: { $gte: startDate, $lte: endDate },
            status: { $ne: 'cancelled' }
        });
        // Generate available time slots
        const availableSlots = generateAvailableSlots(therapist.weeklyAvailability, therapist.blockedSlots, bookings, startDate, endDate);
    ...
    for (let date = new Date(start); date <= end; date.setDate(date.getDate() + 1)) {
        ...
        while (currentTime + slotDuration <= endTime && availableSlotCount < maxSlotsPerDay) {
            ...
            if (!conflictsWithBooking) {
                ...
                const conflictingBooking = blockedPeriods.find(blocked => currentTime < blocked.end && sessionEndTime > blocked.start);
